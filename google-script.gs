...
function doGet(e) {
  ...
  if (action === 'getEvent') {
    ...
    const activeEvent = data.find(row => row[headers.indexOf('is_active')] === true);
    
    if (!activeEvent) return jsonResponse({ event: null });
    
    const eventObj = {};
    headers.forEach((h, i) => eventObj[h] = activeEvent[i]);
    return jsonResponse({ event: eventObj });
  }

  if (action === 'getTopics') {
    const eventId = e.parameter.eventId;
    const topics = getSheetData('topics').filter(t => t.event_id === eventId);
    const profiles = getSheetData('profiles');
    const votes = getSheetData('topic_votes');
    
    const result = topics.map(t => ({
      ...t,
      author: { name: profiles.find(p => p.id === t.author_id)?.name || 'Anonym' },
      votes: votes.filter(v => v.topic_id === t.id)
    }));
  ...
  if (action === 'getDateOptions') {
    const eventId = e.parameter.eventId;
    const options = getSheetData('date_options').filter(o => o.event_id === eventId);
    const votes = getSheetData('date_votes');
    
    const result = options.map(o => ({
      ...o,
      votes: votes.filter(v => v.date_option_id === o.id)
    }));
...
function doPost(e) {
  ...
  if (action === 'login') {
    ...
    const profiles = getSheetData('profiles');
    let profile = profiles.find(p => p.id === email); // Using email as ID for simplicity
  ...
  if (action === 'toggleTopicVote') {
    ...
    const votes = getSheetData('topic_votes');
    const existingIndex = votes.findIndex(v => v.topic_id === data.topicId && v.profile_id === data.profileId);
  ...
  if (action === 'toggleDateVote') {
    ...
    const votes = getSheetData('date_votes');
    const existingIndex = votes.findIndex(v => v.date_option_id === data.optionId && v.profile_id === data.profileId);
  ...
  if (action === 'saveProfile') {
    ...
    const rowIndex = dataRows.findIndex(row => row[idIndex] === data.id);
